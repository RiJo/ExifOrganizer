...
namespace ExifOrganizer.Meta.Parsers
{
    internal class ExifParser : Parser
    {
        ...
        // Exif 2.2 Standard (reference: http://www.exiv2.org/tags.html)
        private enum ExifId : int
        {
        ...
        private static Dictionary<ExifId, object> ParseImage(string filename)
        {
            Dictionary<ExifId, object> data = new Dictionary<ExifId, object>();
        ...
        private static object ParseValue(ExifId id, ExifDataType type, byte[] data)
        {
            ...
            {
                if (data == null)
                    throw new ArgumentNullException(nameof(data), $"ID: {id}. Type: {type}.");
            }
